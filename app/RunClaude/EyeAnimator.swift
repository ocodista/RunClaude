import Foundation

class EyeAnimator: ObservableObject {
    private(set) var currentState: EyeActivityState = .sleeping
    private(set) var animPhase: Double = 0

    /// When set, overrides the auto-detected state (debug use only).
    @Published var forcedState: EyeActivityState? = nil

    private var tokensPerSecond: Double = 0
    private var lastActivityTime: Date = .distantPast

    func updateBurnRate(tokensPerSecond: Double) {
        self.tokensPerSecond = tokensPerSecond

        if tokensPerSecond > 0 {
            lastActivityTime = Date()
        }

        let secondsSinceActivity = Date().timeIntervalSince(lastActivityTime)

        if tokensPerSecond > 2000 {
            currentState = .working
        } else if tokensPerSecond > 10 {
            currentState = .running
        } else if secondsSinceActivity < 30 {
            currentState = .walking
        } else {
            currentState = .sleeping
        }
    }

    func tick() {
        let dt = 1.0 / 24.0

        let speed: Double
        switch currentState {
        case .sleeping: speed = 0.8    // Slow breathing rhythm for Z animation
        case .walking:  speed = 2.8    // ~0.45 Hz walking cadence
        case .running:  speed = 11.0   // ~1.75 Hz running cadence
        case .working:  speed = 20.0   // Blazing fast — running at full tilt
        }

        animPhase += dt * speed
    }
}
